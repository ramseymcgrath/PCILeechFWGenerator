...
module msix_table #(
    parameter NUM_MSIX = 32,                // Number of MSI-X table entries
    parameter MSIX_TABLE_BIR = 4,          // BAR indicator for MSI-X table
    parameter MSIX_TABLE_OFFSET = 32'h00000000,       // Offset of MSI-X table in the BAR
    parameter MSIX_PBA_BIR = 4,            // BAR indicator for MSI-X PBA
    parameter MSIX_PBA_OFFSET = 32'h00000800          // Offset of MSI-X PBA in the BAR
) (
    // Clock and reset
    input  logic        clk,
    input  logic        reset_n,
    ...
    localparam PBA_SIZE = (32 + 31) / 32;
    
    // MSI-X Table storage (4 DWORDs per entry)
    (* ram_style="block" *) logic [31:0] msix_table_mem[0:32*4-1];
    ...
    assign is_table_access = (bar_index == 4) && 
                            (bar_addr >= 32'h00000000) && 
                            (bar_addr < (32'h00000000 + 32 * 16));
                            
    assign is_pba_access = (bar_index == 4) && 
                          (bar_addr >= 32'h00000800) && 
                          (bar_addr < (32'h00000800 + PBA_SIZE * 4));
    
    // Calculate table and PBA addresses
    assign table_addr = (bar_addr - 32'h00000000) >> 2;  // Convert to DWORD index
    assign pba_addr = (bar_addr - 32'h00000800) >> 2;      // Convert to DWORD index
    ...
    // Initialize MSI-X table from provided values
    initial begin
    ...
    // Initialize MSI-X PBA from provided values
    initial begin
