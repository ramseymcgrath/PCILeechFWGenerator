...
def main():
    ...
    try:
        ...
        total_critical = len([i for i in critical_issues if i['severity'] in ['CRITICAL']])
        total_template_critical = len([i for i in template_issues if i['severity'] in ['CRITICAL']])
        ...
        if total_critical > 0 or total_template_critical > 0:
            print(f"❌ CRITICAL ISSUES FOUND: {total_critical + total_template_critical}")
