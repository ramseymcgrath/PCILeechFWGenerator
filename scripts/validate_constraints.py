...
def extract_ports_from_systemverilog(sv_file: Path) -> Set[str]:
    ...
    try:
        ...
        port_lines = re.findall(r'(input|output)\s+logic(?:\s*\[[^\]]+\])?\s+(\w+)', port_section)
        
        for direction, port_name in port_lines:
            ports.add(port_name)
            
            # For bus signals, also check individual bits
            bus_match = re.search(rf'{re.escape(port_name)}\s*\[(\d+):(\d+)\]', port_section)
            if bus_match:
...
def extract_constraints_from_xdc(xdc_file: Path) -> Tuple[Set[str], Set[str]]:
    ...
    try:
        ...
        pin_matches = re.findall(r'set_property\s+PACKAGE_PIN\s+\w+\s+\[get_ports\s+(?:\{)?([^\}]+)(?:\})?\]', content)
        ...
        iostd_matches = re.findall(r'set_property\s+IOSTANDARD\s+\w+\s+\[get_ports\s+([^\]]+)\]', content)
...
def validate_constraints(sv_file: Path, xdc_file: Path) -> bool:
    ...
    for port in required_ports:
        ...
        if any(internal in port for internal in ['bar_', 'cfg_device', 'cfg_class', 'cfg_subsystem']):
            continue
            
        # Check pin assignment
        has_pin = any(port in assigned or port.replace('[', '\\[').replace(']', '\\]') in assigned 
                     for assigned in pin_assignments)
        if not has_pin:
            # Check for wildcard matches
            base_port = port.split('[')[0] if '[' in port else port
            has_wildcard_pin = any(f"{base_port}*" in assigned for assigned in pin_assignments)
            if not has_wildcard_pin:
                missing_pins.append(port)
        
        # Check I/O standard
        has_iostd = any(port in iostd or f"{port.split('[')[0]}*" in iostd 
...
def main():
    ...
    parser.add_argument("--sv-file", type=Path, 
                       help="SystemVerilog top module file (default: search for pcileech_top.sv)")
    parser.add_argument("--xdc-file", type=Path,
                       help="XDC constraints file (default: search for *.xdc)")
    ...
    if not sv_file:
        ...
        if candidates:
            ...
            print("Please specify --sv-file or ensure the file exists in the output directory")
            return 1
    ...
    if not xdc_file:
        ...
        if candidates:
            ...
            print("Please specify --xdc-file or ensure XDC files exist in the output directory")
            return 1
