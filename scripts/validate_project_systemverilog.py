...
def validate_tcl_templates():
    ...
    tcl_files = list(tcl_templates_dir.glob("*.tcl.j2")) + list(tcl_templates_dir.glob("*.tcl"))
    ...
    for tcl_file in tcl_files:
        ...
        target_lang_matches = re.findall(r'set_property\s+target_language\s+(\w+)', content, re.IGNORECASE)
        
        if target_lang_matches:
            for match in target_lang_matches:
                if match.lower() == 'verilog':
                    errors.append(f"{tcl_file}: Uses 'target_language Verilog' - should be 'SystemVerilog'")
        ...
        if verilog_refs:
            for ref in set(verilog_refs):  # Remove duplicates
                warnings.append(f"{tcl_file}: Contains reference to '{ref}' - verify this is intentional")
    ...
    for py_file in python_files:
        ...
        if 'target_language' in content and 'Verilog' in content:
            ...
            if re.search(r'target_language.*["\']Verilog["\']', content):
                errors.append(f"{py_file}: Sets target_language to 'Verilog' - should be 'SystemVerilog'")
