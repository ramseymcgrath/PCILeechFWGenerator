...
def _get_template_render_error_base() -> Type[Exception]:
    ...
    try:
        # Attempt to import from the main exceptions module using the module-level
        # __import__ so tests can patch it (they patch
        # src.templating.template_renderer.__import__).
        mod = __import__("src.exceptions", fromlist=["TemplateRenderError"])
        ImportedError = getattr(mod, "TemplateRenderError")
