...
class VirtualDeviceTable(DataTable):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self.virtual_rows: List["PCIDevice"] = []  # All device rows
        self.visible_start: int = 0  # Start index of visible rows
        self.visible_count: int = 50  # Only render 50 rows at a time

    def update_devices(self, devices: List["PCIDevice"]) -> None:
        """
        Update the device data and render only the visible rows.
    ...
    def _add_device_row(self, device: "PCIDevice") -> None:
        ...
        # Add row with comprehensive device information
        self.add_row(
            status_indicator,
            device.bdf,
    ...
    def get_selected_device(self) -> Optional["PCIDevice"]:
        """
        Get the currently selected device.

        Returns:
            The selected PCIDevice object, or None if no device is selected
        """
        if self.cursor_row is None:
            return None
        ...
        try:
            row_index = self.cursor_row
            if 0 <= row_index < self.row_count:
                row_key = self.get_row_at(row_index)[0]

                # Find the device with the matching BDF (used as row key)
                for device in self.virtual_rows:
