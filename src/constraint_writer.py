...
class ConstraintWriter:
    ...
    def _extract_ports_from_module(self, content: str) -> Set[str]:
        ...
        body_match = re.search(
            r"module\s+pcileech_top\s*\(.*?\);(.*?)endmodule", content, re.DOTALL
        )
        if body_match:
    ...
    def generate_xdc_constraints(self, board: str, verilog_files: List[str]) -> str:
        ...
        logger.info(
            f"Found {len(constrained_ports)} already constrained ports: {sorted(constrained_ports)}"
        )

        # Find unconstrained ports
        unconstrained_ports = all_ports - constrained_ports
        logger.info(
            f"Found {len(unconstrained_ports)} unconstrained ports: {sorted(unconstrained_ports)}"
        )
        ...
        if unconstrained_ports:
            ...
            xdc_lines.append(
                "# These ports need proper pin assignments for your specific board"
            )
            xdc_lines.append("")
    ...
    def generate_constraints_for_build(
        self, board: str, source_files: List[str]
    ) -> str:
        """
        ...
        verilog_files = [f for f in source_files if f.endswith((".sv", ".v", ".vh"))]
