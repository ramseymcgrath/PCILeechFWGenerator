...
class TCLBuilder:
    ...
    def prepare_base_context(
        ...
        # Prepare base context with nested structure to match template expectations
        context = {
            # Nested device information (for master_build.j2 template)
            "device": {
                "vendor_id": f"0x{(vendor_id or config_vendor_id):04x}",
                "device_id": f"0x{(device_id or config_device_id):04x}",
                "class_code": f"0x{config_class_code:06x}",
                "revision_id": f"0x{(revision_id or config_revision_id):02x}",
            },
            # Nested board information (for master_build.j2 template)
            "board": {
                "name": board,
                "fpga_part": fpga_part,
            ...
            # Flat variables for backward compatibility with other templates
            "board_name": board,
            "fpga_part": fpga_part,
            "pcie_ip_type": fpga_config["pcie_ip_type"],
            "fpga_family": fpga_config["family"],
            ...
            "header": f"# PCILeech Firmware Build - {board}",
        }

        # Add hex-formatted IDs for convenience (flat structure)
        context.update(
            {
