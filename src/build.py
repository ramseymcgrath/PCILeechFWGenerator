...
class PCILeechFirmwareBuilder:
    ...
    def _generate_fallback_tcl_files(self, device_info: Dict[str, Any]) -> List[str]:
        ...
        This fallback now attempts to use the template system directly.
        """
        tcl_files = []

        try:
            # Try to use template system directly as fallback
            import time
            ...
            # Build template context similar to TCLGenerator
            ...
            context = {
                "device": {
                    "vendor_id": device_info.get("vendor_id", "0x1234"),
                    "device_id": device_info.get("device_id", "0x5678"),
                    "class_code": device_info.get("class_code", "0x040300"),
                    "revision_id": device_info.get("revision_id", "0x01"),
                },
                "board": {
                    "name": self.board,
            ...
            # Generate master build script using template
            if template_renderer.template_exists("tcl/master_build.j2"):
                master_tcl_content = template_renderer.render_template(
                ...
                logger.info("Generated template-based fallback master build TCL script")
                return tcl_files

        except Exception as e:
            logger.warning(
                f"Template-based fallback failed: {e}, using hardcoded fallback"
            )

        # Ultimate fallback - hardcoded simple script
        ...
        # Generate simple master build script
        master_tcl_content = f"""# PCILeech Master Build Script (Hardcoded Fallback)
# Sources all build scripts in correct order with proper sequencing

puts "Starting PCILeech firmware build process..."
puts "Device: {vendor_id_hex}:{device_id_hex} (Rev: {revision_id_hex})"
...
"""
        ...
        try:
            with open(master_tcl_file, "w", encoding="utf-8") as f:
                f.write(master_tcl_content)
            tcl_files.append(str(master_tcl_file))
            logger.info("Generated hardcoded fallback master build TCL script")
        except Exception as e:
